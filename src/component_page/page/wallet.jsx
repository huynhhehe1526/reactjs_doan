...
const WalletInterface = () => {
  ...
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === "isShowNoticePublish") {
        setIsShowNoticePublish(e.newValue === "true");
      }
    };
    window.addEventListener("storage", handleStorageChange);
    return () => {
  ...
  const handleCreateClick = () => {
    // Redirect to "create" page (you can replace this with the actual page URL you want to create)
    navigate('/create');
  };
  ...
  const handleHideAddressBitcoin = () => {
        ...
        <AppBar position="static" color="transparent" elevation={0}>
          <Container>
            <Box sx={{ display: 'flex', alignItems: 'center', py: 2, justifyContent: 'space-between' }}>
                ...
                >
                  <Flex vertical justify="center" align="center" className="demo">
                    ...
                    >
                      <Popover placement="bottom" title={text} content={content}>
                        <Box sx={{ position: 'relative', display: 'inline-block' }}>
                          <i className="fa-solid fa-bell" style={{ color: "white", fontSize: "20px", cursor: "pointer" }}></i>
                          {notificationCount > 0 && (
                            <Box
                              sx={{
                                position: 'absolute',
                      ...
                      {isWinner && (
                        <Button variant="contained" color="primary" onClick={() => dispatch(getResult())}>
                          Công bố
                        </Button>
                      )}
                    </Flex>
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                <Button
                  variant="contained" color="primary" onClick={handleCreateClick} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}                 >                   Create                 </Button>               </Box>
            </Box>
          </Container>
        </AppBar>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                        ...
                        <Box>
                          ...
                          <Button
                            color="primary"
                            sx={{ p: 0 }}
                            onClick={isShowAddress ? handleHideAddressBitcoin : handleShowAdressBitcoin}
                          >
                            {isShowAddress ? (
                              <Box>
                                <Typography variant="body2">{bitcoinWallet}</Typography>
                                <Button
                                  color="secondary"
                                  sx={{ mt: 1 }}
                                  onClick={handleHideAddressBitcoin}
                                >
                                  Ẩn
                                </Button>
                              </Box>
